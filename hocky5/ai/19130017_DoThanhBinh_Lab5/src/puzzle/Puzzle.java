...
public class Puzzle {
	...
	public int computeH2(Node currentState) {
		int result = 0;
		/* Enter your code here */
		for (int i = 1; i < MAX_COL * MAX_ROW; i++) {
			
				result += PuzzleUtils.manhattanDistance(currentState.getLocation(i),
						goalState.getLocation(i));
	...
	public Node moveWhiteTile(Node currentState, char operator) {
		...
		if (operator == 'u') {// Case-1: Move tile UP
			...
			if (row < MAX_ROW) {// Tile stands inside the map
				int tmp = currentState.getTile(row, col);
				result.updateTile(row, col, 0); // change position
				result.updateTile(whiteTile[0], whiteTile[1], tmp); // change position
				result.setH(computeH2(result));
				return result;
			}
		}
